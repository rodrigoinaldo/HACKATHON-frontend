...
const Insurt: React.FC = () => {
  const { type } = useParams<{ type: string }>(); // Obtém o tipo de conteúdo da URL
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('http://127.0.0.1:8000/api/bazzar', {
        method: 'POST',
        body: formData,
      });
      ...
      if (response.ok) {
        setImage(null);
        setSelectedImage(null); // Limpa a visualização da imagem
  ...
  const handleImageChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (event.target.files && event.target.files[0]) {
      ...
      setImage(file); // Armazena o arquivo para envio
    }
  };

  return (
    <div>
      <h1>Adicionar bazzar</h1>
      {message && <p>{message}</p>}
      
        {/* Renderize diferentes campos com base no tipo */}
        {type === 'bazzar' && (
          <>
